...
namespace LiveSplit.Subnautica
{
    public class SubnauticaMemory : Memory
    {
        ...
        private bool IsFlareThrowDrop()
        {
            ...
            {
                logger.Log("Ignored Flare drop while PDA inventory was open.");
                return false;
            }

            bool flareHeldOrWasHeld = string.Equals(ActiveToolName.New, "flare", StringComparison.OrdinalIgnoreCase)
                || string.Equals(ActiveToolName.Old, "flare", StringComparison.OrdinalIgnoreCase);
            if (!flareHeldOrWasHeld)
            {
                logger.Log("Ignored Flare drop because the active tool was not a Flare.");
                return false;
            }

            logger.Log("Flare drop detected outside PDA inventory.");
            return true;
        }
